...
To add support for a DAQmx device that is not yet supported, run update_capabilities.py
on a computer with the device in question connected (or with a simulated device of the
...
devices whose capabilities are known, run generate_classes.py. Subvlasses of NI_DAQmx
will be made in this folder, and they can then be imported into labscript code as:
...
be installed vi pip (Python 3.6+ only). If you don't want to install this library have
this library, the code will still work, it just won't be formatted well.